  ...
  <div class="add-customer-contract-body">
    ...
    <div class="add-customer-contract">
      ...
      <div class="customer-card">
        <span>销课时间</span>
        <el-date-picker
          v-model="sureClass.sureClassTime"
          type="datetime"
...
import {apiUpdateSureClass} from '../../api/customerSureClass'
...
export default {
  ...
  methods: {
    hiddenDialog () {
      this.$emit('update:modifySureClassFlag', false)
    },
    saveCustomerContract () {
      console.log(this.sureClass)
      if (!this.sureClass.sureClassTime) {
        this.$message.warning('请选择销课日期')
        return
      }
      apiUpdateSureClass(this.sureClass).then(() => {
        this.success()
        this.hiddenDialog()
      })
    },
